            ...
            if (p.isVivant()) {
                if (p.estMonstre()) {
                    ...
                    switch (p.getDernDirection()) {
                        case DROITE:
                            if (p.isInvulnerable()) {
                                BufferedImage filteredImage = filtregold(imagesDroite.get(compteur));
                                g.drawImage(filteredImage, 0, 0, getWidth(), getHeight(), this);
                            } else {
                                // Réinitialiser l'image à sa version originale
                                resetFiltre(imagesDroite.get(compteur), imagesDroite.get(compteur));

                                // Dessiner l'image normale si l'objet n'est pas invulnérable
                        ...
                        case GAUCHE:
                            if (p.isInvulnerable()) {
                                BufferedImage filteredImage = filtregold(imagesGauche.get(compteur));
                                g.drawImage(filteredImage, 0, 0, getWidth(), getHeight(), this);
                            } else {
                                // Réinitialiser l'image à sa version originale
                                resetFiltre(imagesGauche.get(compteur), imagesGauche.get(compteur));
                        ...
                        case BAS:
                            if (p.isInvulnerable()) {
                                BufferedImage filteredImage = filtregold(imagesBas.get(compteur));
                                g.drawImage(filteredImage, 0, 0, getWidth(), getHeight(), this);
                            } else {
                                // Réinitialiser l'image à sa version originale
                                resetFiltre(imagesBas.get(compteur), imagesBas.get(compteur));
                        ...
                        case HAUT:
                            if (p.isInvulnerable()) {
                                BufferedImage filteredImage = filtregold(imagesHaut.get(compteur));
                                g.drawImage(filteredImage, 0, 0, getWidth(), getHeight(), this);
                            } else {
                                // Réinitialiser l'image à sa version originale
                                resetFiltre(imagesHaut.get(compteur), imagesHaut.get(compteur));
